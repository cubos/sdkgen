module AST
  class DateTimePrimitiveType
    ...
    def typescript_encode(expr)
      "(typeof #{expr} === \"string\" && (#{expr} as any).match(/^[0-9]{4}-[01][0-9]-[0123][0-9]T[012][0-9]:[0123456][0-9]:[0123456][0-9].[0-9]{1,3}Z?$/) ? (#{expr} as any).replace(\"Z\", \"\") : #{expr}.toISOString().replace(\"Z\", \"\"))"
    end
